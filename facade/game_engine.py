"""The game engine by Farseen Ashraf"""
import time
...
class GameEngine:
    ...
    def submit_entry(cls, user_id:str, entry: Decimal) -> bool:
        ...
        time_remaining = cls._start_time - now + cls._clock
        if time_remaining > 0:
            if Wallets.get_balance(user_id=user_id) > Decimal('1'):
                if Wallets.adjust_balance(user_id=user_id, amount=Decimal('-1')):
                    cls._entries.append((user_id, entry))
                    Reports.log_event(f"New entry '{entry}' submitted by '{user_id}'")
                    return True
                Reports.log_event(f"Problem Adjusting balance for '{user_id}'")
                return False
            Reports.log_event(f"Balance for '{user_id}' is too low")
            return False
